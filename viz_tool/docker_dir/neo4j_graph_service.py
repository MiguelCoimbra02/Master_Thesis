...
# Function to load CSV into Neo4j
    ...
    def search_gene_in_neo4j(self, query_nodes):
        ...
        #DELETE THIS LATER
        with self.get_session() as session:
            cypher_query = "MATCH (n:GeneNode) RETURN count(n) AS gene_node_count"
            result = session.run(cypher_query)
            record = result.single()  # Get single result
            gene_node_count = record["gene_node_count"] if record else 0
            print(f"Number of GeneNode nodes: {gene_node_count}")
